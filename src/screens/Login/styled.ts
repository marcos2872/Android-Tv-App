...
interface propsType { theme: DefaultTheme, focus: boolean }
...
justify-content: center;
border-radius: 5px;
background-color: ${({ theme, focus }: propsType) => focus ? theme.colors.button : theme.colors.dbutton};
`
export const Register = styled.TouchableOpacity`
`
export const Text = styled.Text`
color: ${({ theme }) => theme.colors.text};
font-size: ${({ text }: { text: string }) => text === 'entrar' ? RFPercentage(2) : RFPercentage(1)}px;
`
